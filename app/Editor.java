...
public class Editor extends JFrame implements RunnerListener {
  ...
  static JMenu midiMenu;
  static JMenu midiInputMenu;
  static JMenu midiOutputMenu;
  static boolean midiMenuInitialized = false;

  static MidiInputMenuListener midiInputMenuListener;
  static MidiOutputMenuListener midiOutputMenuListener;
...
//    sketchMenu.add(item);
  ...
  class MidiMenuListener implements ActionListener {
    protected String prefName = "midi.port";
    ...
    public void actionPerformed(ActionEvent e) {
      if (getMenu() == null) {
        System.out.println("midiMenu is null");
        return;
      }
 				
      JMenu midiMenu = getMenu();
      int count = midiMenu.getItemCount();
      for (int i = 0; i < count; i++) {
        if (midiMenu.getItem(i) instanceof JCheckBoxMenuItem) {
          ((JCheckBoxMenuItem)midiMenu.getItem(i)).setState(false);
        }
      }
      ...
      Preferences.set(prefName, name);
      System.out.println("MIDI " + prefName + " set to " + Preferences.get(prefName));
				
    }
  }
 		
  class MidiInputMenuListener extends MidiMenuListener {
    protected JMenu getMenu() {
      return midiInputMenu;
    }
 			
    MidiInputMenuListener() {
      super();
      prefName = "midi.inputport";
    }
  }
 		
  class MidiOutputMenuListener extends MidiMenuListener {
    protected JMenu getMenu() {
      return midiOutputMenu;
    }
 			
    MidiOutputMenuListener() {
      super();
      prefName = "midi.outputport";
    }
  }
  ...
  protected void populateMidiMenu() {
    ...
    MidiUploader uploader = new MidiUploader();
    
    rbMenuItem = new JMenuItem("Rescan MIDI Ports");
    rbMenuItem.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent e) {
          SwingUtilities.invokeLater(new Runnable() {
              public void run() {
                midiMenuInitialized = false;
                populateMidiMenu();
              }
            });
        }
      });
    midiMenu.add(rbMenuItem);
    midiMenu.addSeparator();
    
    if (midiInputMenu != null) {
      midiInputMenu.removeAll();
    } else {
      midiInputMenu = new JMenu("Input port");
    }
    if (midiOutputMenu != null) {
      midiOutputMenu.removeAll();
    } else {
      midiOutputMenu = new JMenu("Output port");
    }
    
    String ports[];
    
    ports = uploader.getMidiInputPorts();
    
    if (ports != null) {
      System.out.println("ports input " + ports.length);
      for (int i = 0; i < ports.length; i++) {
        String curport = ports[i];
        rbMenuItem = new JCheckBoxMenuItem(curport, curport.equals(Preferences.get("midi.inputport")));
        rbMenuItem.addActionListener(midiInputMenuListener);
        //serialGroup.add(rbMenuItem);
        midiInputMenu.add(rbMenuItem);
        empty = false;
      }
    }
    ...
    ports = uploader.getMidiOutputPorts();
    if (ports != null) {
      System.out.println("ports output " + ports.length);
      for (int i = 0; i < ports.length; i++) {
        String curport = ports[i];
        rbMenuItem = new JCheckBoxMenuItem(curport, curport.equals(Preferences.get("midi.outputport")));
        rbMenuItem.addActionListener(midiOutputMenuListener);
        //serialGroup.add(rbMenuItem);
        midiOutputMenu.add(rbMenuItem);
        empty = false;
      }
    }
...
//      code.undo = new UndoManager();
  ...
  protected boolean handleOpenInternal(String path) {
    ...
    setTitle(sketch.getName() + " | " + Base.PROGRAM_NAME + " " + Base.VERSION_NAME);
