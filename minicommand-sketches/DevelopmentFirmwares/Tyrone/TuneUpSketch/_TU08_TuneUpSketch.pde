...
class TuneUpSketch : public Sketch, public MidiCallback  {
    ...
    void setupKeys() {
      for (int i = 0; i < NUM_KEYS; ++i) {
        setupScaleMap(keys[i].m_scale12, keys[i].m_scale);
        keyNames[i] = keys[i].m_name;
      }
    }
    ...
    bool allowedNotesMatchScale() {
      scale_t* curScale = &keys[keysEncoder.getValue()].m_scale;
      for (int i = 0; i < NUM_ALLOWED_NOTES; ++i) {
        if (allowedNotes[i]) {
    ...
    uint8_t* getScale() {
      uint8_t value = keysEncoder.getValue();
      return keys[value].m_scale12;
    }
    ...
    void setup() {
      ...
      keysEncoder.initEnumEncoder(keyNames, NUM_KEYS);
      keysEncoder.setName("Key");      
      noteEncoder.setName("Pit"); 
      encoderPage1.setup();
      encoderPage1.setShortName("Str");
      //EncoderPage encoderPage1(&transActivateEncoder, &chordActivateEncoder, &keysEncoder, &noteEncoder);
      encoderPage1.setEncoders(&transActivateEncoder, &chordActivateEncoder, &keysEncoder, &noteEncoder);
      ...
      Midi2.addOnNoteOnCallback(this, (midi_callback_ptr_t)&TuneUpSketch::onNoteOn);
      Midi2.addOnNoteOffCallback(this, (midi_callback_ptr_t)&TuneUpSketch::onNoteOff);
      Midi2.addOnControlChangeCallback(this, (midi_callback_ptr_t)&TuneUpSketch::onControlChange);
