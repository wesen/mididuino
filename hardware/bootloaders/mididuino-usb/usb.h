...
/**## USB stack prototypes
 **/
uint8_t usb_config_ep(uint8_t config0, uint8_t config1);
void usb_init(void);
void usb_start_device(void);
uint8_t usb_init_device(void);
uint8_t usb_select_endpoint_interrupt(void);
uint8_t usb_send_packet(uint8_t ep, uint8_t *buf, uint8_t len);
uint8_t usb_read_packet(uint8_t ep, uint8_t *rbuf, uint8_t len);
void usb_halt_endpoint(uint8_t ep);
void usb_main(void);

typedef void(*usb_handler_callback_t)(void);

typedef struct usb_handler_s {
  usb_handler_callback_t sof;
  usb_handler_callback_t wake_up;
  usb_handler_callback_t resume;
  usb_handler_callback_t suspend;
  usb_handler_callback_t reset;
  usb_handler_callback_t vbus_on;
  usb_handler_callback_t vbus_off;
  usb_handler_callback_t set_configuration;
} usb_handler_t;
...
uint8_t usb_write_pkt_to_ep(uint8_t ep, uint8_t *msg, uint16_t length);
uint8_t usb_read_pkt_from_ep(uint8_t ep, uint8_t *buf, uint8_t length);
